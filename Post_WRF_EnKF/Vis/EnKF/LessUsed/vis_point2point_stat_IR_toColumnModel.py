...
def corr_snapshot( wrf_dir,DAtime,var_name,var_dim,ver_coor=None):
    ...
    if var_dim == '2D':
        ...
        if interp_H and not interp_P:
            # interpolate
            Interp_corr_xb_hxb = stats.vertical_interp( ncdir,corr_colxb_hxb,ver_coor,interp_H) 
            if If_plot_corr_snapshot:
                plot_3Dcorr_snapshot( xlat[idx_xb],xlon[idx_xb],Interp_corr_xb_hxb,ver_coor )
        elif interp_P and not interp_H:
             
            if If_plot_corr_snapshot: 
                plot_3Dcorr_snapshot( xlat[idx_xb],xlon[idx_xb],Interp_cov_xb_hxb,ver_coor )
        else:
            pass
...
def stddev_xb_snapshot( wrf_dir,DAtime,var_name,var_dim,ver_coor=None):
    ...
    if var_dim == '2D':
        ...
        if interp_H and not interp_P:
            # interpolate
            Interp_cov = stats.vertical_interp( ncdir,stddev_xb,ver_coor,interp_H)
