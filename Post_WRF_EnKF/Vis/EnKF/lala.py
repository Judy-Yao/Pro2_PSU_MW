...
def cross_Tb_toCol( xb_ens,hxb_ens ):
    ...
    for n in prange( xb_ens.shape[2] ): # loop thru samples
        for m in range( xb_ens.shape[1] ): 
            for l in range( xb_ens.shape[0] ): 
...
def plot_snapshot( lat,lon,Interp_cov,ver_coor ):
    ...
    # Define the colorbar
    max_abs = 0.000003 #max(np.amin(abs(Interp_incre)),np.amax(abs(Interp_incre)))
    min_var = -0.000003#0-max_abs
    ...
    for isub in range(6):
        ...
        cs = ax.flat[isub].scatter(lon,lat,3,Interp_cov[isub,:],cmap='RdBu_r',edgecolors='none',transform=ccrs.PlateCarree(),)
        #cs = ax.flat[isub].scatter(lon,lat,3,Interp_cov[isub,:],cmap='RdBu_r',edgecolors='none',vmin=min_var,vmax=max_abs,transform=ccrs.PlateCarree(),)
        #cs = ax.flat[isub].scatter(lon,lat,3,Interp_cov[isub,:]*(d_all['Yo_obs']-d_all['meanYb_obs']),cmap='RdBu_r',edgecolors='none',transform=ccrs.PlateCarree(),)
        if any( hh in DAtime[8:10] for hh in ['00','06','12','18'] ):
    ...
    #fig.suptitle(Storm+':'+Exper_name+'~kgain', fontsize=10, fontweight='bold')
...
def cal_2Dcov_IR_ColVar( DAtime, var_name):
    ...
    # calculate K
    #kgain = np.divide(cov_xb_hxb,stddev_hxb**2)/(3**2)
    #cov_xb_hxb = kgain
