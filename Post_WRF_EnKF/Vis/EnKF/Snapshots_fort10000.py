...
def Diagnose(big_dir, Storm, Exper, DAtime, v_interest, Check_IR, Check_MW):
    ...
    # Check each assimilated variables
    key_obstype = ['slp','IR', 'MW']
    Diag_obs_types = {key: None for key in key_obstype}
    
    if Check_slp:
        Diag_obs_types['slp'] = Diag.Find_min_slp( file_diag, v_interest )
        Plot_slp( Storm, DAtime, Exper, Diag_obs_types )
...
if __name__ == '__main__':
    ...
    for DAtime in DAtimes:
        ...
        Diagnose(big_dir, Storm, Exper_name, DAtime, v_interest, Check_slp, Check_IR, Check_MW)
