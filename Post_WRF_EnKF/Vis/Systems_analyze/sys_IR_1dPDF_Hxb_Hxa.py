...
def Plot_hist_IRsum( d_hcount ):
            ...
            for ida in DA:
                for key in d_hcount[imp][ida]:
                    ...
                    if 'b' in key:
                        hx=r'$\mathbf{\overline{H(Xb)}}$'
                        if imp == 'WSM6':
                            axs.flat[0].plot(x_axis,hist,color=csm_color(imp,ida),linewidth='4',label='WSM6')
                            axs.flat[0].plot(x_axis, fitted, 'k', linewidth=2, linestyle='--',label='Gauss_WSM6')
                            #axs.flat[0].text(0.53, 0.70, 'WSM6 Mode: '+format(mu, '.2f'), transform=axs.flat[0].transAxes,fontsize=12)
                        else:
                            axs.flat[0].plot(x_axis,hist,color=csm_color(imp,ida),linewidth='4',label='Tune',alpha=0.6)
                            axs.flat[0].plot(x_axis, fitted, 'k', linewidth=2, linestyle='--',label='Gauss_Tune',alpha=0.5)
                            #axs.flat[0].text(0.53, 0.6, 'Tune Mode: '+format(mu, '.2f'), transform=axs.flat[0].transAxes,fontsize=12)
                        ...
                        if imp == 'WSM6':
                            ...
                            axs.flat[1].plot(x_axis,hist,color=csm_color(imp,ida),linewidth='4',label='Tune',alpha=0.6)
    ...
    for j in range(2):
        ...
        if bin_Tbdiff:
            axs.flat[j].set_ylim(ymin=0,ymax=0.30)
        ...
        axs.flat[j].set_ylabel('Density',fontsize=15)
