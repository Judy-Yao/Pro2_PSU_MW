...
def read_IC_water( wrf_dir ):
    ...
    for i in range(len(mean_dir)):
        ...
        IC_water[i,:,:] = np.sum(vapor_content,axis=0)/9.8
...
def read_UV10_slp( wrf_dir ):
    ...
    for i in range(len(mean_dir)):
        ...
        slp_smooth[i,:,:] = sp.ndimage.gaussian_filter(slp, [11,11])
        # UV10
        U10[i,:,:] = ncdir.variables['U10'][0,:,:]
        V10[i,:,:] = ncdir.variables['V10'][0,:,:]
        windspeed[i,:,:] = (U10[i,:,:] ** 2 + V10[i,:,:] ** 2) ** 0.5

    d_UV10_slp = {'lat':lat,'lon':lon,'slp_smooth':slp_smooth,'U10':U10,'V10':V10,'windspeed':windspeed}
    return d_UV10_slp
...
def plot_UV10_slp( Storm, Exper_name, DAtime, wrf_dir, plot_dir ):
    ...
    # Title
    ax[0].set_title( 'Xb--min slp: '+str("{0:.3f}".format(np.amin( slp_smooth[0,:,:] )))+' hPa',  fontweight='bold') #, fontsize=12)
    ax[1].set_title( 'Xa--min slp: '+str("{0:.3f}".format(np.amin( slp_smooth[1,:,:] )))+' hPa',  fontweight='bold') #, fontsize=12)
