...
def plot_slp( Storm, Exper_name, DAtime, wrf_dir, plot_dir ):
    ...
    for i in range(2):
        ...
        # Mark the best track
        if if_btk_exist:
            ax[i].scatter(dict_btk['lon'][idx_btk],dict_btk['lat'][idx_btk], 20, 'black', marker='*',transform=ccrs.PlateCarree())
            ax[i].text(dict_btk['lon'][idx_btk]-1,dict_btk['lat'][idx_btk]-0.5,dict_btk['min_slp'][idx_btk],fontsize=10)
