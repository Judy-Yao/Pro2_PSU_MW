...
def plot_hpi(Storm, Exper_name, wrf_dir, read_HPI_wrfout, domain_range, output_dir=None):
    reload_data = False
    step = 1
    ...
    if read_HPI_wrfout == True:
        ...
        for init_time in DF_init_times:
            plot_one( ax0, ax1, ax2, read_wrfout(Storm, Exper_name, wrf_dir+'/'+Storm+'/'+Exper_name+'/wrf_df/'+init_time, force_reload=reload_data), color_model[i], 'IR: '+ init_time, step=step)
            i = i+1
    elif read_HPI_wrfout == False:
        i = 0
        for init_time in DF_init_times:
            plot_one( ax0, ax1, ax2, read_rsl_error(Storm, Exper_name, wrf_dir+'/'+Storm+'/'+Exper_name+'/wrf_df/'+init_time, init_time, DF_model_end), color_model[i], 'IR: '+ init_time, step=step)
            i = i+1
    ...
    if output_dir is not None:                             
        plt.savefig('/work2/06191/tg854905/stampede2/Pro2_PSU_MW/'+Storm+'/'+Exper_name+'/Vis_analyze/Model/'+Storm+'.png')
    else:
        plt.show()
    plt.clf()
