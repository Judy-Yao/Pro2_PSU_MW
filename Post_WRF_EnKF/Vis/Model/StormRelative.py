...
def plot_azmean( var_name,d_var,d_coor  ):
    ...
    cbar = fig.colorbar(xa,cax=cbaxes,fraction=0.046, pad=0.04)
    ...
    for idx in range(2):
        ...
        ax.flat[idx].set_xticklabels(  [str(it) for it in xlabel_like],fontsize=15 )
        ax.flat[idx].set_xlabel('Radius (KM)',fontsize=20)
