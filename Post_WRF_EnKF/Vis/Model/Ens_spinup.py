...
def identify_mpsfc_ens( wrf_dir, key=None):
    ...
    if If_save:
        ...
        if key == 'start':
            des_path = wrf_dir+ start_time_str+'_ens_mpsfc.txt'
        else:
            des_path = wrf_dir+ end_time_str+'_ens_mpsfc.txt'
...
def identify_mslp_ens( wrf_dir, key=None):
    ...
    if If_save:
        ...
        if key == 'start':
            des_path = wrf_dir+ start_time_str+'_ens_mslp.txt'
        else:
            des_path = wrf_dir+ end_time_str+'_ens_mslp.txt'
        with open(des_path,'w') as f:
            # Add header 
            f.write('\t'.join( item.rjust(6) for item in header ) + '\n' )
            # Write the record to the file serially
...
def Read_vars_Ens( var_name ):
    ...
    for imp in MP:
        d_st[imp] = {}
        d_ed[imp] = {}
        wrf_dir = big_dir+Storm+'/'+Exper_name[imp]+'/fc/'+start_time_str+'/'
        if var_name == 'PSFC':
            print('Reading the minimum PSFC for each ens member...')
            txt_file = wrf_dir+ start_time_str+'_ens_mpsfc.txt'
            d_st[imp] = read_mpsfc_ens( txt_file )
            txt_file = wrf_dir+ end_time_str+'_ens_mpsfc.txt'
            d_ed[imp] = read_mpsfc_ens( txt_file )
        elif var_name == 'slp':
            print('Reading the mslp for each ens member...')
            txt_file = wrf_dir+ start_time_str+'_ens_mslp.txt'
            d_st[imp] = read_mslp_ens( txt_file )
            txt_file = wrf_dir+ end_time_str+'_ens_mslp.txt'
...
def plot_geo_move( d_st, d_ed ):
    ...
    sm.set_array([])  # Dummy array for colorbar
    cax_below = fig.add_axes([0.15, 0.92, 0.7, 0.02])  # [left, bottom, width, height]
