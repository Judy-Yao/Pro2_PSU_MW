...
def compute_hydro_area( wrf_files, hydros):
    ...
    # Find the min slp in enkf output as the anchor point
    output_file = wrf_files[1]
    ncdir = nc.Dataset( output_file )
    slp_original = UD.compute_slp( ncdir ).flatten()
    min_slp = np.amin( slp_original )
    idx_anchor = np.where(slp_original == min_slp)[0]
    # Find the circled area with the min slp as the center 
    # & read the common variables: xlat/xlon
    xlat = ncdir.variables['XLAT'][0,:,:].flatten()
    xlon = ncdir.variables['XLONG'][0,:,:].flatten()
    anchor_ij = ll_to_xy(ncdir, xlat[idx_anchor], xlon[idx_anchor])
    idx_area = UD.find_circle_area( output_file, anchor_ij.values[0], anchor_ij.values[1], 200, model_resolution/1000) # in km
     #print(len(idx_area))
...
# Plot height profile of the sum of hydrometeors
def plot_PF_all(DAtime, plot_dir, d_hydro, Height):
    ...
    ax.set_xticklabels( ['0','250','500','750','1000','1250','1500','2000'],fontsize=15 ) 
    ...
    # Set title
    title_name = 'Profile: mass of hydrometeors in the area'
...
if __name__ == '__main__':
    ...
    each_water = True
    Plot_PF = True
    Plot_all = True # all species together
    # -------------------------------------------------------    

    if not Consecutive_times:
        DAtimes = ['201709041400','201709041500']
    ...
    if Plot_PF:
        ...
        for DAtime in DAtimes:
            ...
            d_hydro,Height = compute_hydro_area( wrf_files, hydros)
            # ------ Plot -------------------
            plot_dir = small_dir+Storm+'/'+Exper_name+'/Vis_analyze/Model/VProfile_Hydro_mass/'
            ...
            if each_water:
                ...
                if not Plot_all:
                    for var in v_interest:
                        print('Plotting the profile of '+var)
                        #PF_xb = np.sum(d_hydro[var][0,:,:],axis=1) 
                        #PF_xa = np.sum(d_hydro[var][1,:,:],axis=1)
                        plot_PF_each(DAtime, plot_dir, d_hydro, var )
                else:
                   plot_PF_all(DAtime, plot_dir, d_hydro, Height ) 
            else:
                pass
