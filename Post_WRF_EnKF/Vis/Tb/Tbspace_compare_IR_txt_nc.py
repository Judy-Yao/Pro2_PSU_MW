...
def plot_Tb(Storm, Exper_name, DAtime):
    ...
    #print( np.amin(d_simu['Lat_x']), np.amax(d_simu['Lat_x']), np.amin(d_simu['Lon_x']), np.amax(d_simu['Lon_x']))
    #print( np.amin(d_obs['Lat_obs']), np.amax(d_obs['Lat_obs']), np.amin(d_obs['Lon_obs']), np.amax(d_obs['Lon_obs']) )

    #lat_min = np.amin(d_obs['Lat_obs'])
    #lat_max = np.amax(d_obs['Lat_obs'])
    #lon_min = np.amin(d_obs['Lon_obs'])
    #lon_max =  np.amax(d_obs['Lon_obs'])
    ...
    ax[1].scatter(d_simu['Lon_x'], d_simu['Lat_x'],1,c=d_simu['Yb_x'],\
    ...
    plt.savefig('/work2/06191/tg854905/stampede2/Pro2_PSU_MW/'+Storm+'/'+Exper_name+'/Vis_analyze/Tb/IR/'+DAtime+'_g16_ch8_Tbspace.png', dpi=300)
