...
def plot_Tb( Storm, Exper_name, Hxb, FCtime):

    # Read simulated data
    d_simu = read_simu_IR_one( Hxb, ch_list )

    # Read location from TCvitals
    #if any( hh in FCtime[8:10] for hh in ['00','06','12','18']):
    #    tc_lon, tc_lat = UD.read_TCvitals(small_dir+Storm+'/TCvitals/'+Storm+'_tcvitals', FCtime)
...
if __name__ == '__main__':
    ...
    # ---------- Configuration -------------------------
    Storm = 'IRMA'
    Exper_name = 'wsm6_atvs6_alpha05_n0smax1e12'
    sensor = 'abi_gr'
    ch_list = ['8',]
    ...
    if If_plot:
        for FCtime in IR_times:
            ...
            print('------------ Plot ----------------------')
            for ifile in file_y:
                plot_Tb( Storm, Exper_name, ifile, FCtime,) # d_obs)
