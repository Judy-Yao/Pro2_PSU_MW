...
def write_mean_bin ( DAtime, sensor, Hx_dir, ch_list):
    ...
    with open(file_name,'w') as f:
        # Add header 
        f.write('\t'.join( item.rjust(4) for item in header ) + '\n' )
        ...
        for irow in range( len_records ):
            ...
            f.write('\t'.join( item.rjust(4) for item in irecord ) + '\n')
...
def plot_Tb(Storm, Exper_name, Hx_dir, DAtime, sensor, ch_list ):
    ...
    cs = ax[1].scatter(d_all['lon_obs'][idx_x], d_all['lat_obs'][idx_x],1.5,c=d_all['meanYb_obs'][idx_x],\
    ...
    if not limit:
        des_name = small_dir+Storm+'/'+Exper_name+'/Vis_analyze/Tb/IRch8_Obspace/'+DAtime+'_bfABEI_'+sensor+'_Obspace.png'
    else:
        des_name = small_dir+Storm+'/'+Exper_name+'/Vis_analyze/Tb/IRch8_Obspace/'+DAtime+'_bfABEI_'+sensor+'_Obspace_limit.png'
