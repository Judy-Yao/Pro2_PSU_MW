...
def write_mean_eachSensor( Hx_dir, sensor ):
    ...
    file_yb = sorted( glob.glob(Hx_dir + '/input_*' + sensor + '*.txt') ) 
    file_ya = sorted( glob.glob(Hx_dir + '/output_*' + sensor + '*.txt') )
...
def read_Tb(Tb_file, sensor, dict_ss_len):
    ...
    for ifile in Tb_file:
        ...
        #tmp = np.fromfile( ifile, sep=' ' )
        #print(tmp)
        #print(np.size( tmp[0::ncol] ))
        #lat_obs = lat_obs + tmp[0::ncol]
        #lon_obs = lon_obs + tmp[1::ncol]  
        #ch_obs = ch_obs + [ int(item) for item in tmp[2::ncol] ]
        #Yo_obs = Yo_obs + tmp[3::ncol]
        #meanYb_obs = meanYb_obs + tmp[4::ncol]
        #meanYa_obs = meanYa_obs + tmp[5::ncol]  

    print(np.size(lat_obs))
    print(dict_ss_len[sensor])
...
def plot_Tb(Storm, Exper_name, DAtime, sensor, dict_ss_len):
    ...
    # Define the domain
    lat_min = np.amin(d_all['lat_obs'].flatten())
    lat_max = np.amax(d_all['lat_obs'].flatten())
    lon_min = np.amin(d_all['lon_obs'].flatten())
    lon_max = np.amax(d_all['lon_obs'].flatten())
    ...
    for DAtime in MW_times:
        ...
        for sensor in dict_ss_ch:
            ...
            if sensor == 'gmi_gpm':
                ...
                dict_ss_len['gmi_gpm_lf'] = len_pre_processed 
