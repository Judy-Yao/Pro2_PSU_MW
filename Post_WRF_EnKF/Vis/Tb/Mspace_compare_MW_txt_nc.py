...
def plot_Tb(Storm, Exper_name, DAtime, sensor):
    ...
    for input_it in range(2):
        ...
        if sensor == 'gmi_gpm':
            ...
            #    is_ocean = globe.is_ocean(Lat_x_ch, Lon_x_ch)  
            #    mask_x = is_ocean
            #else:
            #    mask_x = np.full((np.size(Lat_x_ch), ), True)
            ...
            #ax[i,1].scatter(Lon_x_ch[mask_x], Lat_x_ch[mask_x],2.5,c=Yb_x_ch[mask_x],\
            #    edgecolors='none', cmap=MWJet, vmin=min_T, vmax=max_T, transform=ccrs.PlateCarree()) 
            ax[i,1].scatter(Lon_x_ch, Lat_x_ch,2.5,c=Yb_x_ch,\
                edgecolors='none', cmap=MWJet, vmin=min_T, vmax=max_T, transform=ccrs.PlateCarree()) 
            if any( hh in DAtime[8:10] for hh in ['00','06','12','18']):
            ...
            #cs = ax[i,2].scatter(Lon_x_ch[mask_x], Lat_x_ch[mask_x],2.5,c=Ya_x_ch[mask_x],\
            #    edgecolors='none', cmap=MWJet, vmin=min_T, vmax=max_T, transform=ccrs.PlateCarree())
            cs = ax[i,2].scatter(Lon_x_ch, Lat_x_ch,2.5,c=Ya_x_ch,\
                 edgecolors='none', cmap=MWJet, vmin=min_T, vmax=max_T, transform=ccrs.PlateCarree())
            if any( hh in DAtime[8:10] for hh in ['00','06','12','18']):
            ...
            #    mask_x = is_ocean
            #else:
            #    mask_x = np.full((np.size(Lon_x_ch), ), True)


            #if d_simu['Ch_x'][Ch_idx] == d_lowf[sensor]:
            #    is_ocean = globe.is_ocean(d_simu['Lat_x'], d_simu['Lon_x'])
            #    mask_x = is_ocean
            #else:
            #    mask_x = np.full((np.size(d_simu['Lon_x'], ), True)
    ...
    ax[0,1].set_title(r'$H_{Tb}(Xb)$', font, fontweight='bold')
    ax[0,2].set_title(r'$H_{Tb}(Xa)$', font, fontweight='bold')
    ...
    plt.savefig('/work2/06191/tg854905/stampede2/Pro2_PSU_MW/'+Storm+'/'+Exper_name+'/Vis_analyze/Tb/MW/Tbspace/'+DAtime+'_'+sensor+'_Tbspace.png', dpi=300)
...
if __name__ == '__main__':
    ...
    #MW_times = ['201709170700','201709170900','201709171000','201709171100','201709171300','201709171700']
