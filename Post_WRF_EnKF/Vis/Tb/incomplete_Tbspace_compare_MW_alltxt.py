...
for i in range(2):
 ...
 #   ax[i,0].coastlines(resolution='10m', color='black',linewidth=0.5)
    cs = ax[i,0].scatter(Lon_all[idx],Lat_all[idx],scatter_size[i],c=Yo_all[idx],edgecolors='none', cmap=newJet, vmin=min_T, vmax=max_T) #,i transform=ccrs.PlateCarree())
 ...
 #   ax[i,1].coastlines(resolution='10m', color='black',linewidth=0.5)
    cs = ax[i,1].scatter(Lon_all[idx],Lat_all[idx],scatter_size[i],c=Yb_all[idx],edgecolors='none', cmap=newJet, vmin=min_T, vmax=max_T)#, transform=ccrs.PlateCarree()) 
