...
def plot_2by2_byColor():
    ...
    colorset = storm_color()
    alphas = alpha_fc_init()
    colorset= {'CONV':'black','IR':'red','MW':'blue'}   #DA_marker()
...
def plot_3by2():
    ...
    alphas = alpha_fc_init()
    DA_color= {'CONV':'black','IR':'red','MW':'blue'} 
    ...
    for imp in MP:
        ...
        for ist in Storms:
            ...
            for fc_init in fc_inits:
                fsp_track = fsp_ir[ist][imp]['MW'][fc_init][0]
                fsp_mslp = fsp_ir[ist][imp]['MW'][fc_init][1] # Vmax: [2]
    ...
    for imp in MP:
        ...
        for ist in Storms:
            ...
            for fc_init in fc_inits:
                fsp_track = fsp_ir[ist][imp]['MW'][fc_init][0]
                fsp_mslp = fsp_ir[ist][imp]['MW'][fc_init][1] # Vmax: [2]
                #print(ist,imp,fc_init)
                ax['fsp_ir'][imp].scatter(fsp_mslp,fsp_track,s=30,marker='o',facecolor=storm_colors[ist],alpha=alphas[fc_inits.index(fc_init)],label='_nolegend_')

    # Add legends
    # DAs
    scatter_DA = ax['fsp'][MP[1]].collections
    legend_DA = ax['fsp'][MP[1]].legend([scatter_DA[i] for i in [3,7]],['IR','MW'],fontsize='10',loc='lower right')
    # Add the first legend manually to the current Axes
...
def plot_2by2_MAEs():
    ...
    for imp in MP:
        ...
        if imp == 'WSM6':
            ...
            fig.text(0.53,0.30,'MSLP: MAE (hPa)', fontsize=8, ha='center', va='center',rotation='vertical')
...
def plot_1by2_FSP():
    ...
    for imp in MP:
        for ist in Storms:
            ...
            for fc_init in fc_inits:
                fsp_track = fsp[ist][imp]['MW'][fc_init][0]
                fsp_mslp = fsp[ist][imp]['MW'][fc_init][1] # Vmax: [2]
