...
#import EnKF_mslp_track as SC #StormCenter
...
def plot_hist_1by2():
    ...
    grids = fig.add_gridspec(ncols=1,nrows=1,) #hspace=0.12
    ax = {}
    for iv in var_its:
    ...
    lines = {'CONV':'-','IR':(0,(5,1)),'MW':(0,(3,1,1,1,1,1))}
    ...
    for iv in var_its:
        for imp in MP:
            for ida in DA:
                ...
                if ida == 'CONV':
                     ax[iv].plot(x_axis,hist_var[imp][ida][iv]['hist'],color=colors[imp],linestyle=lines[ida],linewidth='4',alpha=0.3,label=imp+'_'+ida)
                elif ida == 'IR':
                     ax[iv].plot(x_axis,hist_var[imp][ida][iv]['hist'],color=colors[imp],linestyle=lines[ida],linewidth='4',alpha=alphas[imp],label=imp+'_'+ida)
                else:
                    ax[iv].plot(x_axis,hist_var[imp][ida][iv]['hist'],color=colors[imp],linestyle=lines[ida],linewidth='4',alpha=alphas[imp],label=imp+'_'+ida)
    ...
    for iv in var_its:
        ...
        ax[iv].set_ylim([-0.001,0.30])
        # Set tick label font size
        ax[iv].tick_params(axis='both', which='major', labelsize=15) 


    # Set label names
    fig.text(0.03,0.5,'Probability Distribution Function', fontsize=15, ha='center', va='center',rotation='vertical')
    for iv in var_its:
        if iv == 'track':
            ax[iv].set_xlabel( 'Track Bias (km)',fontsize = 15 )
        elif iv == 'mslp':
            ax[iv].set_xlabel( 'MSLP Bias (hPa)',fontsize = 15 )
        elif iv == 'vmax':
...
def plot_EnKF_HPI():
    ...
    # Save figure
    des_name = small_dir+'/Clean_results/SYSTEMS/Vis_analyze/Paper1/EnKF_HPI.png'
    plt.savefig( des_name )
    print( 'Saving the figure to '+des_name )
...
def plot_Precip():
    ...
    # Save figure
    des_name = small_dir+'/Clean_results/SYSTEMS/Vis_analyze/Paper1/Cycle_ave_Precip.png'
    plt.savefig( des_name )
    print( 'Saving the figure to '+des_name )
        ...
        for imp in MP:
            ...
            for ida in DA:
                ...
                if not os.path.exists( big_dir+'/'+ist+'/'+Exper_names[ist][imp][ida] ):
                    ...
                    for iv in var:
                        if iv == 'vmax':
                            d_model[ist][imp][ida][iv] = model_hpi['xa_vmax']
                            ...
                            #d_minslp = SC.model_mslp(big_dir,ist,Exper_names[ist][imp][ida],d_hrs[ist],slp_xa,slp_xb )
                            if iv == 'mslp':
                                d_model[ist][imp][ida][iv] = model_hpi['xa_mslp']
                                #d_model[ist][imp][ida][iv] = d_minslp['xa_mslp']
                            elif iv == 'track':
                                d_model[ist][imp][ida][iv] = [ model_hpi['lon'], model_hpi['lat'] ]
        ...
        for istorm in Storms:
            ...
            for iv in var:
                if iv == 'vmax':
                    ...
                    if if_tcvital:
                        d_tcvital= SC.assimilated_obs( big_dir,small_dir,istorm,Exper_names[istorm]['WSM6']['CONV'], d_hrs[istorm],slp_xa,slp_xb=False )
                        if iv == 'mslp':
