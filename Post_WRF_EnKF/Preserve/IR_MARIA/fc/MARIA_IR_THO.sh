...
#SBATCH -p icx-normal
#SBATCH -t 15:00:00
#SBATCH -o out_fc_MARIA_IR_THO
#SBATCH -e error_fc_MARIA_IR_THO
...
DA=IR
MP=THO
Exper=IR-J_DA+J_WRF+J_init-SP-intel17-THO-24hr-hroi900
# Specify time range
spinup_st=201709151200
date_st=201709161600
date_ed=201709162300
...
while [[ $DAtime -le $date_ed ]]; do
  ...
  # Synoptic times 
    ...
    bdy_files=($(find . -maxdepth 1 -type f -name 'wrfbdy*'))
    start=`date +%s.%N`
    if [[ ${DAtime} -ne 201709160600 ]]; then
      tar -czvf ${DAtime}_wrfbdy_d01.tar.gz "${bdy_files[@]}"
    fi
    end=`date +%s.%N`
    runtime=$( echo "$end - $start" | bc -l )
  ...
  # other regular times
    ...
    # wrf_enkf_input_d03_0*
    #!!!!!!!!!!!!!!!!!
    if [ ${DAtime} -ne 201709161600 ]; then
      wrf_files=($(find . -maxdepth 1 -type f -name 'wrf_enkf_input_d03*'))
    fi
    start=`date +%s.%N`
    tar -czvf ${DAtime}_wrf_enkf_input_d03.tar.gz "${wrf_files[@]}"
