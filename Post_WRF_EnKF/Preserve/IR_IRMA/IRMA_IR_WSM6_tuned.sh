...
#SBATCH -p icx-normal
#SBATCH -t 20:00:00
#SBATCH -o out_IRMA_IR_tune
#SBATCH -e error_IRMA_IR_tune
...
if $at_run; then
  ...
  # tar
  start=`date +%s.%N`
  tar -czvf ${Storm}_${DA}_${MP}_tune_run.tar.gz run 
  end=`date +%s.%N`
...
if ${at_wrf_df}; then
  ...
  times=$(ls wrf_df_origin)
  # enter wrf_df directory
  cd wrf_df_origin; echo 'Enter wrf_df_origin'
  ...
  cd /scratch/06191/tg854905/Pro2_PSU_MW/${Storm}/${Exper}
  # list initialization times
  times=$(ls wrf_df_modify)
  # enter wrf_df directory
  cd wrf_df_modify; echo 'Enter wrf_df_modify'
  # compress each forecast
  for it in ${times}; do
    # clean
    cd ${it}; echo 'clean '${it}
