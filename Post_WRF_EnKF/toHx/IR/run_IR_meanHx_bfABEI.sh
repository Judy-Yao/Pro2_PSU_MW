...
while [[ $DAtime -le $date_ed ]]; do
  ...
  if [[ ! -d ${Big_dir}/${Storm}/${Exper}/fc/${DAtime}_bfABEI ]]; then
    break
  fi
    
  # check if output dir exists
  outdir=${Big_dir}/${Storm}/${Exper}/Obs_Hx/IR/${DAtime}_bfABEI
  ...
  for istate in "${state[@]}"; do
    ...
    for mem in `seq -f "%03g" 1 $nE`; do
      wrffile=${Big_dir}/${Storm}/${Exper}/fc/${DAtime}_bfABEI/wrfinput_d0"$dom"_${year}-${month}-${day}_${hour}:${minute}:00_$mem 
