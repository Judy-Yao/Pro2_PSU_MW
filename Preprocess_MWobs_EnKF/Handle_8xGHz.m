function [Swath_used,ChIdx_all,ChName_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime] = Handle_8xGHz(iTb,sensor,Swath_used,ChIdx_all,ChName_all,DAtime_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime,control)
	...
	% ========================================================================
		...
		if (~control.use8xGHz)
            if num_89GHz_scan == 2
        	    lat{idx_89GHz(1)} = []; lat{idx_89GHz(2)} = []; 
    	        lon{idx_89GHz(1)} = []; lon{idx_89GHz(2)} = [];
        	    Tb{idx_89GHz(1)} = []; Tb{idx_89GHz(2)} = [];
            	zenith{idx_89GHz(1)} = []; zenith{idx_89GHz(2)} = []; 
            	sat_lat{idx_89GHz(1)} = []; sat_lat{idx_89GHz(2)} = [];
            	sat_lon{idx_89GHz(1)} = []; sat_lon{idx_89GHz(2)} = [];
            	sat_alt{idx_89GHz(1)} = []; sat_alt{idx_89GHz(2)} = [];
            	sat_azimuth{idx_89GHz(1)} = []; sat_azimuth{idx_89GHz(2)} = []; 
            	outime{idx_89GHz(1)} = []; outime{idx_89GHz(2)} = []; 
                ...
                sat_lon{idx_89GHz(1)} = [];
                sat_alt{idx_89GHz(1)} = [];
                sat_azimuth{idx_89GHz(1)} = [];
                outime{idx_89GHz(1)} = [];

                Swath_used{iTb}(idx_89GHz(1)) = [];
                ChIdx_all{iTb}(idx_89GHz(1)) = [];
                ChName_all{iTb}(idx_89GHz(1)) = [];
		...
		elseif (control.use8xGHz) & control.comnine_AMSR89GHz % 89-GHz data of AMSR2 is in use.
			...
			if num_89GHz_scan ~= 2
                ...
                % Rename the combined 89GHz scan
                idx_Ascan = idx_89GHz(idx_89GHz ~= idx_Bscan);
                ChName_AB = ChName_all{iTb};
                ChName_AB(idx_Ascan) = "89GHzV";
                ChName_all{iTb} = ChName_AB;
            end

            for i_sw = 1:length(Swath_used{iTb})
                disp(['        Updated AMSR2 swath: '+Swath_used{iTb}(i_sw)]);
                disp(['        Updated AMSR2 channel name: '+ChName_all{iTb}(i_sw)]);
            end
	    else
            return            
    	end
