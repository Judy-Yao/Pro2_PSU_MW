	...
	if contains(filext,"HDF5")
		...
		for i = 1:length(fileheader)
			if contains(fileheader{i},'StartGranuleDateTime') 
				start_time_print = extractBefore(erase(fileheader{i},'StartGranuleDateTime='),18); % only save year-month-date-hour-minutes
				disp(['    Start Granule Time: ',strtrim(start_time_print)]);
			elseif contains(fileheader{i},'StopGranuleDateTime') 
				end_time_print = extractBefore(erase(fileheader{i},'StopGranuleDateTime='),18); % only save year-month-date-hour-minutes
				disp(['    End Granule Time: ',strtrim(end_time_print)]);
			end
		end
		% convert saved time strings to scalar datetime arrays
		start_time = datetime(strtrim(start_time_print),'InputFormat','yyyy-MM-dd''T''HH:mm', 'TimeZone','UTC');
		end_time = datetime(strtrim(end_time_print),'InputFormat','yyyy-MM-dd''T''HH:mm', 'TimeZone','UTC'); 
	% NC file
	elseif contains(filext,"nc")
        start_time_print = extractBefore(ncreadatt(Tb_file,'/','time_coverage_start'),17);
        disp(['    Start Granule Time: ',strtrim(start_time_print)]);
        start_time = datetime(start_time_print,'InputFormat','yyyy-MM-dd''T''HH:mm', 'TimeZone','UTC');

        end_time_print = extractBefore(ncreadatt(Tb_file,'/','time_coverage_end'),17);  
        disp(['    End Granule Time: ',strtrim(end_time_print)]);
		end_time = datetime(end_time_print,'InputFormat','yyyy-MM-dd''T''HH:mm', 'TimeZone','UTC');
	end
