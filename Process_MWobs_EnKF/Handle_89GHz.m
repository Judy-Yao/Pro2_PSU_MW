function [Swath_used,ChIdx_all,ChName_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime] = Handle_89GHz(iTb,sensor,Swath_used,ChIdx_all,ChName_all,DAtime_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime,control)
	...
	% ========================================================================
		...
		elseif (control.use89GHz) & control.comnine_AMSR89GHz % 89-GHz data of AMSR2 is in use.
    ...
    % ========================================================================
        ...
        if (~control.use89GHz)
            ...
            ChIdx_all{iTb}(idx_89GHz) = [];
            ChName_all{iTb}(idx_89GHz) = [];
            disp('        89 GHz of SSMI is not in use!');
		else	
			disp('        89 GHz of SSMI is used!');
			return;	
		end

	else
		error('        Error: double check if this sensor has 89 GHz!');
	end
