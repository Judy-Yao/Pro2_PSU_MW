...
function [Swath_used,ChIdx_all,ChName_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime] = Handle_AMSR2(iTb,Swath_used,ChIdx_all,ChName_all,DAtime_all,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime)
    ...
    if control.NOTuse_AMSR89GHz & (~control.comnine_AMSR89GHz) % 89-GHz data of AMSR2 is not in use.
        for ith = 1:length(idx_89GHz) 
            ...
            lon{idx_89GHz(ith)} = [];
            Tb{idx_89GHz(ith)} = [];
            zenith{idx_89GHz(ith)} = [];
            sat_lat{idx_89GHz(ith)} = [];
            sat_lon{idx_89GHz(ith)} = [];
            sat_alt{idx_89GHz(ith)} = [];
    ...
    elseif (~control.NOTuse_AMSR89GHz) & control.comnine_AMSR89GHz % 89-GHz data of AMSR2 is in use.
        if num_89GHz_scan ~= 2
            ...
            if DAtime_all{iTb}(idx_89GHz(1)) == DAtime_all{iTb}(idx_89GHz(2)) %if DA_time is the same for both scans, combine them
                ...
                disp('(sanity check) size of the final 89 GHZ scan: ',num2str(size(lat{1})));
