function [Swath_used,ChIdx_ps,ChName_ps,Clat,Clon,CTb,Czenith,Csat_lat,Clon_sat,Csat_alt,Csat_azimuth,Coutime] = Combine_AMSR2(iTb,Swath_used,ChIdx_ps,ChName_ps,lat,lon,Tb,zenith,sat_lat,sat_lon,sat_alt,sat_azimuth,outime)
...
% In other words, under such an assumption, 89GHz B scan just provides more measurements of the same kind data to 89GHz A scan
    ...
    for i_sw = 1:length(Swath_used{iTb})
        if contains(ChName_ps{i_sw},'89GHzV-PolA-Scan') | contains(ChName_ps{i_sw},'89GHzV-PolB-Scan')
    ...
    if num_89GHz_scan ~= 2
        ...
        if DAtime_perSwath{sw_89GHz(1)} == DAtime_perSwath{sw_89GHz(2)}
            ...
            for ith = 1:length(idx_89GHz)
                if contains(ChName_ps{idx_89GHz(ith)},'89GHzV-PolB-Scan')
