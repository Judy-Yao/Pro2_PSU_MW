...
!-- use_radiance
   ...
   use_aoei_radiance = .false.
   use_elf_radiance = .false.
   use_inf_bt_radiance = .false.
...
!========================================================================================
subroutine get_wrf_info(wrf_file, ix, jx, kx, ks, times, proj)
...
!-----------------------------------------------------------------------------
   ...
   integer, intent(out)            :: ix, jx, kx, ks
...
!-----------------------------------------------------------------------------
   ...
   call get_ij ( wrf_file, ix, jx, kx, ks)
   if ( my_proc_id == 0 ) write(6,'(3(a7,i3))')"   ix :",ix,"   jx :",jx,"   kx:",kx,"   ks:",ks
...
!========================================================================================
subroutine rd_truth_wrf(wrf_file,ix,jx,kx,ks,ni,nj,nk,nv,nm,gid,sid,iid,jid,x)
   use namelist_define
   use wrf_tools
   use mpi_module
   implicit none
   ...
   character (len=10), intent(in) :: wrf_file
   integer, intent(in)  :: ix, jx, kx,ks,ni,nj,nk,nv,nm,gid,sid,iid,jid 
   real, dimension(ni,nj,nk,nv), intent(out) :: x
   integer  :: ii, jj, kk, m, istart,iend,jstart,jend
   real, allocatable, dimension(:,:,:) :: dat3d

   do m=1,nv
      call wrf_var_dimension ( wrf_file, enkfvar(m), ix, jx, kx, ks, ii, jj, kk )
      istart=iid*ni+1
      iend=(iid+1)*ni
      jstart=jid*nj+1
      jend=(jid+1)*nj
...
!========================================================================================
subroutine read_ensemble(i_unit,ix,jx,kx,ks,ni,nj,nk,nv,nm,gid,sid,iid,jid,x)
use namelist_define
use wrf_tools
use mpi_module
implicit none
integer, intent(in)  :: i_unit,ix,jx,kx,ks,ni,nj,nk,nv,nm,gid,sid,iid,jid
...
!========================================================================================
subroutine output(o_unit,ix,jx,kx,ks,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,xout,times)
use namelist_define
use netcdf
use wrf_tools
use mpi_module
implicit none
integer, intent(in) :: o_unit,ix,jx,kx,ks,ni,nj,nk,nv,nm,sid,iid,jid,s_comm
...
do m=1,nv
  call wrf_var_dimension(filename,enkfvar(m),ix,jx,kx,ks,ii,jj,kk)
  allocate(dat3d(ii,jj,kk))
...
!==============================================================================
subroutine wrf_var_dimension ( wrf_file, var, ix, jx, kx, ks, ii, jj, kk )
   use netcdf

   character (len=*), intent(in) :: wrf_file
   character(len=10), intent(in)   :: var
   integer, intent(in)             :: ix, jx, kx, ks
...
! Helper functions for xb.f microwave subroutines
 ...
 FUNCTION beam_conv_simple(numx_crtm, numy_crtm, Tb, obs_ii, obs_jj, dx_wrf, dy_wrf, efov_a, efov_p)
   ...
   !WRITE(*,*) '  numx_crtm and numy_crtm: ', numx_crtm, numy_crtm
   !WRITE(*,*) '  efov_multiplier: ', efov_multiplier
   !WRITE(*,*) efov_a, efov_p, dx_wrf, dy_wrf, pi
   !WRITE(*,*) sqrt(efov_a*efov_p/dx_wrf/dy_wrf/pi)
   ...
   !write(*,*) 'distance_x: ', distance_x
   !write(*,*) 'distance_y: ', distance_y
   ...
   !if (weight > 0.) then
   !  write(*,*) '  at x=',xx,' y=',yy,' weight=',weight
   !endif
   !if (Tb(xx,yy) .EQ. Tb(xx,yy)) then
   !  write(*,*) '  at x=',xx,' y=',yy,' Tb NOT NaN'
   !endif
 ...
 FUNCTION beam_conv_gaussian_simple(numx_crtm, numy_crtm, Tb, obs_ii, obs_jj, dx_wrf, dy_wrf, efov_a, efov_p)
   ...
   !WRITE(*,*) '  in beam_conv_gaussian_simple for x=', obs_ii, ', y=', obs_jj
   !WRITE(*,*) '  numx_crtm and numy_crtm: ', numx_crtm, numy_crtm
   !WRITE(*,*) '  ', efov_a, efov_p, sigma
   ...
   !if (weight > 0.) then
   !  write(*,*) '  x=',xx,' y=',yy,' distance=', distance, 'weight=',weight
   !endif
   !if (Tb(xx,yy) .EQ. Tb(xx,yy)) then
   !  write(*,*) '  at x=',xx,' y=',yy,' Tb NOT NaN'
   !endif
   if (distance .LE. max_distance) then
     !write(*,*) '  x=',xx,' y=',yy,' weight=',weight,' Tb=',Tb(xx,yy)
     Tb_conv = Tb_conv + Tb(xx,yy) * weight
     if (Tb(xx,yy) .NE. Tb(xx,yy)) then
       write(*,*) '  at x=',xx,' y=',yy,' Tb is ', Tb(xx,yy)
       write(*,*) '    beam_conv_simple is now =',(Tb_conv / weight_sum)
