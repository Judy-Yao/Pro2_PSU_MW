...
if ( .not. use_ideal_obs ) then
   if (my_proc_id==0) write(*,*) 'raw%gts%num: ', raw%gts%num
   do n = 1, raw%gts%num
...
allocate( obs%efov_aScan    ( iobs    ) )
allocate( obs%efov_cScan    ( iobs    ) )
allocate( obs%scan_angle    ( iobs    ) )
allocate( obs%zenith_angle  ( iobs    ) )
allocate( obs%azimuth_angle ( iobs    ) )
...
character (len=12)                       :: platform_read !sbs May 2020
...
enddo
if (my_proc_id==0) write(*,*) 'sort_sounding_data sta: ', sta
...
enddo
if (my_proc_id==0) write(*,*) 'sort_upperair_data sta: ', sta
...
enddo
if (my_proc_id==0) write(*,*) 'sort_surface_data sta: ', sta
allocate(slp(sta,3))
allocate(pw(sta,3))
