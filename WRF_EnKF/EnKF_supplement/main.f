...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer            :: unit
  integer            :: ie, ic, it, ix, jx, kx, ks, len, i, j, k, n, ntot, ni,nj,nk
...
! ( reference to MM5_to_GrADS: module_wrf_to_grads_util.f)
  ...
  call get_wrf_info(wrf_file, ix, jx, kx, ks, times, proj) ! saved wrf info to proj 
...
call read_ensemble(i_unit,ix,jx,kx,ks,ni,nj,nk,nv,nm,gid,sid,iid,jid,x)
...
enddo
if(gid==0) &
  call output(i_unit+numbers_en+1,ix,jx,kx,ks,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,xm,times)
...
!!! BT-based inflation if use_inf_bt_radiance by Minamide 2017.9.21
if (use_inf_bt_radiance .and. (raw%radiance%num.ne.0)) then
  if ( my_proc_id == 0 ) write(*,*)'Running BT-based inflation'
  write(wrf_file,'(a5,i5.5)')'fort.',i_unit+1
  call cal_inflate_bt(wrf_file,ix,jx,kx,ks,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)

  if ( my_proc_id == 0 ) write(*,*)'Output members and mean now...'
  do n =1, nm
    ie = (n-1)*nmcpu+gid+1
    if ( ie <= numbers_en ) then
      call output(i_unit+ie,ix,jx,kx,ks,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,x(:,:,:,:,n),times)
    endif
  enddo
...
!!! BT-based inflation if use_inf_bt_microwave by <not yet made>
if (use_inf_bt_microwave .and. (raw%microwave%num.ne.0)) then
  if ( my_proc_id == 0 ) write(*,*) 'Microwave BT-based inflation not supported'
endif
!!! BT-based inflation end
...
write(wrf_file,'(a5,i5.5)')'fort.',i_unit+1
call enkf(wrf_file,ix,jx,kx,ks,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
time2 = MPI_Wtime()
...
do n =1, nm
  ...
  if ( ie <= numbers_en+1 ) then
    call output(o_unit+ie,ix,jx,kx,ks,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,x(:,:,:,:,n),times)
  endif
enddo
