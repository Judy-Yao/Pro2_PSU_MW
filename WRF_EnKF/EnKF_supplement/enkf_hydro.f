...
if(raw%radiance%num.ne.0) then
  ...
  do ie = 1, numbers_en+1
    ...
    xbsend_slp_tb=0.
    write( filename, '(a5,i5.5)') wrf_file(1:5), iunit+ie-1
    call xb_to_radiance(filename,proj,ix,jx,kx,xlong,xlat,times,xland,iob_radmin,iob_radmax,yasend_tb,xbsend_slp_tb)
...
!--ensemble loop for satellite microwave
...
if(raw%microwave%num.ne.0) then
  ...
  do ie = 1, numbers_en+1
    ...
    xbsend_slp_tb=0.
    write( filename, '(a5,i5.5)') wrf_file(1:5), iunit+ie-1
    call xb_to_microwave(filename,proj,ix,jx,kx,xlong,xlat,xland,times,iob_MWmin,iob_MWmax,yasend_tb,xbsend_slp_tb,.true.)
    yasend(iob_MWmin:iob_MWmax,ie) = yasend_tb(iob_MWmin:iob_MWmax)
    xbsend_slp(iob_MWmin:iob_MWmax,ie) = xbsend_slp_tb(iob_MWmin:iob_MWmax)
  enddo
endif
!write(*,*)'yasend',yasend(:,1)
call MPI_Allreduce(yasend,ya,obs%num*(numbers_en+1),MPI_REAL,MPI_SUM,comm,ierr)
...
! --- for Observation Error Inflation
   ...
   if ((obstype=='Radiance  ' .and. use_aoei_radiance) .or. (obstype=='Radiance  ' .and. use_aoei_microwave)) then 
...
!   ngx = obs%roi(iob,1)
   if ((obstype=='Radiance  ' .or. obstype=='Microwave ') .and. &
...
!     update_flag = 0
!   else if ((obstype /= 'Radiance  ' .or. obstype=='Microwave ') .and.  &
...
!         if ((obstype=='Radiance  ' .or. obstype=='Microwave ') .and. (xb_slp(iob,ie)<985.0) .and. (ya(iob,ie)>230.0)) then
...
!         endif
     ...
     enddo
!     if ((obstype=='Radiance  ' .or. obstype=='Microwave ') .and. (xb_slp(iob,numbers_en+1)<985.0) .and.(ya(iob,numbers_en+1)>230.0)) then
...
!   if ((obstype=='Radiance  ' .or. obstype=='Microwave ') .and. (abs(y_hxm)>max(error*3.,sqrt(fac *var))))then
