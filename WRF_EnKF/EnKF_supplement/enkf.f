...
!
!--------------------------------------------------------------------------------------------------------------
subroutine enkf (wrf_file,ix,jx,kx,ks,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
use constants
use namelist_define
use obs_define
use mpi_module
...
integer :: iob_radmin,iob_radmax,iob_MWmin,iob_MWmax
...
if(raw%radiance%num.ne.0) then
  ...
  do ie = 1, numbers_en+1
    ...
    if (ie == numbers_en+1 .and. use_elf_radiance) then
...
if (use_elf_radiance) call MPI_Allreduce(yasend_nc,ya_nc,obs%num,MPI_REAL,MPI_SUM,comm,ierr)
...
!--ensemble loop for microwave brightness temperatures
 !---every grid is calculated in subroutine xb_to_microwave
if(raw%microwave%num.ne.0) then
  yasend_tb=0.
  do ie = 1, numbers_en+1
    yasend_tb = 0.0
    write( filename, '(a5,i5.5)') wrf_file(1:5), iunit+ie-1
    call xb_to_microwave(filename,proj,ix,jx,kx,xlong,xlat,xland,times,iob_MWmin,iob_MWmax,yasend_tb,.true.,.true.)
    yasend(iob_MWmin:iob_MWmax,ie) = yasend_tb(iob_MWmin:iob_MWmax)
  enddo
endif
...
! 0. get basic information of the obs being assimilated
   ...
   if( abs(y_hxm)>(error*5.) .and. &
      .not.(obstype=='min_slp   ' .or. obstype=='longtitude' .or. obstype=='latitude  ' .or. obstype=='slp       '&
       !.or. obstype=='Radiance  ' .or. obstype=='Microwave ' .or. obstype(1:9)=='Psounding') ) then
...
!
   ...
   ngx = obs%roi(iob,1)
   if (obstype=='Radiance  ' .or. obstype=='Microwave ') then
     ...
     if ((obstype=='Radiance  ' .and. use_aoei_radiance) .or. (obstype=='Microwave ' .and. use_aoei_microwave)) then 
     ...
     !if ((obstype=='Radiance  ' .and. use_elf_radiance) .or. (obstype=='Microwave ' .and. use_elf_microwave)) then 
     if (obstype=='Radiance  ' .and. use_elf_radiance) then
...
if((raw%radiance%num+raw%microwave%num).ne.0) then
 do m=1,nv
   varname=enkfvar(m)
   xq_p = 0.
   xq_n = 0.
