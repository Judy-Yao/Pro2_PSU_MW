...
! ----------------------------------------
subroutine cal_inflate_bt (wrf_file,ix,jx,kx,ks,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
use constants
use namelist_define
use obs_define
use mpi_module
...
integer, intent(in) :: ix,jx,kx,ks,ni,nj,nk,nv,nm, g_comm,s_comm
...
real, dimension(ni,nj,nk)  :: inflate_bt
real  :: infalte_tmp
...
ya_ca = abs(ya-ya_nc) - abs(obs%dat-ya_nc)
...
! 0. get basic information of the obs being assimilated
   ...
   if (obstype=='Radiance  ') then
      call get_inflate_facotr_bt(ya_ca(iob),infalte_tmp)
      y_hxm_inf = infalte_tmp - inflate_bt(int(obs%position(iob,1)),int(obs%position(iob,2)),int(obs%position(iob,3)))
      if ( my_proc_id==0 ) write(*,'(a,i6,a,f6.2,a,f6.2,a,f6.2,a,f6.2)') &
'No.',iob,' '//obstype//'(inf) =',infalte_tmp,', ca=', ya_ca(iob),', inf_b=',infalte_tmp-y_hxm_inf,', delta_inf=',y_hxm_inf
      ngx = obs%roi(iob,3)
      ngz = obs%roi(iob,2)
      ...
      call wrf_var_dimension ( wrf_file, varname, ix, jx, kx, ks, ii, jj, kk )
      if ( kk == 1 ) then
        kst  = 1
        ked = 1
      endif
      ...
      do i = uist,uied
        ...
        km(i-uist+1,j-ujst+1,k-kst+1) = corr_coef
      enddo
      enddo
      enddo
      ...
      if( ist<=iend .and. ied>=istart .and. jst<=jend .and. jed>=jstart ) then
        inflate_bt (max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                    max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1, kst:ked) = &
        inflate_bt (max(ist,istart)-istart+1:min(ied,iend)-istart+1, &
                    max(jst,jstart)-jstart+1:min(jed,jend)-jstart+1, kst:ked) + km * y_hxm_inf
      endif
      deallocate(km)
   endif
end do obs_assimilate_cycle

if ( my_proc_id==0 ) write(*,*)'inf_mean= ',sum(inflate_bt(:,:,1))/float(ni*nj),'inf_max= ',maxval(inflate_bt)


!!! adaptive update of overall inflation factor 
if (use_adaptive_bt_inf) then
  inflate_update = inflate
  ! read previous time cycle estimation
  ...
  if( iost .eq. 0 ) then
    ...
    inflate_update = inflate_update
    if (inflate_update .lt. min_inflate) inflate_update = min_inflate
    if (inflate_update .gt. max_inflate) inflate_update = max_inflate
    if ( my_proc_id == 0 ) write(*,*) 'inflation factor from previous cycle=',inflate_update
  endif
  close(10)

  ! homogeneous multiplication of spatially-varying inflation factor
  inflate_bt = inflate_bt * inflate_update / (sum(inflate_bt(:,:,1))/float(ni*nj))
endif
!!!
...
if ( my_proc_id==0 ) write(*,*)'final inf_mean= ', sum(inflate_bt(:,:,1))/float(ni*nj)


if(my_proc_id==0) then
open(10,file='fort.20000',form='unformatted',access='direct',recl=1)
irec = 0
do j = 1,nj
  do i = 1,ni
    irec = irec + 1
    write( 10, rec=irec) inflate_bt(i,j,1)
  enddo
enddo
close (10)
...
do n = 1, nm
   ie = (n-1)*nmcpu+gid+1
   do m=1,nv
      if ( ie<=numbers_en+1 ) x(:,:,:,m,n)=inflate_bt*(x(:,:,:,m,n)-xm(:,:,:,m)) + xm(:,:,:,m)
   enddo
enddo
...
end subroutine cal_inflate_bt

!==============================================================================
subroutine get_inflate_facotr_bt(ca,inf)
...
!real, parameter     :: a = 0.015 !GOES-16
real, parameter     :: a = 0.014 !GOES-13
!real, parameter :: min_inflate = 1.
!real, parameter :: max_inflate = 1.4
