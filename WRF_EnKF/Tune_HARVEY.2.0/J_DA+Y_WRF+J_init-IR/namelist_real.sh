...
debug_level        = 0,
force_use_old_data = .true., ! if set to .true., allow WRF Version 3 input data
EOF
...
echo "&dynamics"
cat << EOF
hybrid_opt          = 0, !Deal with error: Error : Cannot use old input and try to use hybrid vertical coordinate option       
use_theta_m         = 0, !If the user has older input data (pre v4.0), and if the user turns off the moist theta option, then the wrfinput_d0x and wrfbdy_d01 data may be used 
w_damping           = 0,
use_input_w         = .true.,
diff_opt            = 2,
km_opt              = 4,
