...
export CONFIG_FILE=/expanse/lustre/projects/pen116/zuy121/Pro2_PSU_MW/SourceCode/WRF_EnKF/IRMA_run/IR-THO_firstIRthenTCvitals/config_IR_THO.IRMA.expanse
...
while [[ $NEXTDATE -le $DATE_CYCLE_END ]]; do
  ...
  if [ "$DATE" -ge "$DATE_CYCLE_START" ] && [ "$DATE" -le "$DATE_CYCLE_END" ]; then
    ...
    "$SCRIPT_DIR"/module_enkf_part1.sh
    "$SCRIPT_DIR"/module_enkf_part2.sh
  fi
